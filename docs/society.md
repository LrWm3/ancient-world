...
- **Terrain routes:** [GPU frontier searches](gpu-navigation.md) connect each new site to up to three nearby prior sites on the same inner continent. Paths follow adjacent dry central-land cells, with slope and river-crossing costs. Markets and relief use these routes when the layer is enabled. Route closure prevents new departures; cargo already traveling continues. Only direct surveyed links are used, without network itineraries, ports or intercontinental shipping. Founding migration remains the older annual abstraction.
...
The optional [political extension](politics.md) now adds parent/child records, marriages, child inheritance, council factions, inner-continent territorial administration and campaigns. The limitations above describe the base social layer when that extension is disabled.
