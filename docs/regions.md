...
Relaxation is numerical equilibrium iteration, not physical elapsed time or a shallow-water velocity solver. It stops when per-pass depth changes are below `max(0.0005 m, 5e-7 * max(abs(bed), abs(depth)))`; an edge-head deadband of comparable scale avoids floating-point chatter. This is a local update tolerance, not a promise of one globally uniform surface to 0.5 mm. The solver uses the configured lake iteration budget: automatic mode allows max(16,384, terrain face resolution²) passes; explicit budgets are validated multiples of 16. It reports iteration counts and wall time and raises an error if unresolved. See [lake polling review](lake-polling-review.md) for current polling controls and measured results. The greater geographic fidelity costs more passes than the old shared-volume approximation. The removed basin-wide reduction buffer saves 32 bytes per planet cell (192 MiB at 1024² per face).
