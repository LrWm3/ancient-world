...
fn month(@builtin(global_invocation_id) g:vec3<u32>) {
 ...
 var cultivated=min(s.habitat.y,select(s.stock.x*select(.45,.465,p.options.x==2u),workers(i,s.stock.x)*worker_shares(economies[i],s.stock.x,workers(i,s.stock.x)*(1.-LAND_RECOVERY_WORK_PENALTY*select(0.,clamp(economies[i].soil.w,0.,1.),(p.options.w&2u)!=0u))).x*CULTIVATED_HECTARES_PER_WORKER_MONTH,(p.options.w&1u)==1u)); // ha, capped by available labor
